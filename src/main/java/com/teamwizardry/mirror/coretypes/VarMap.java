...
class VarMap {
    ...
    AnnotatedType map(AnnotatedType type, MappingMode mappingMode) {
        if (type.getType() instanceof Class) {
            ...
            Annotation[] merged = GenericTypeReflector.merge(type.getAnnotations(), tv.getAnnotations(), map.get(tv).getAnnotations(), varFromClass.getAnnotations());
            return GenericTypeReflector.updateAnnotations(map.get(tv), merged);
	        ...
	        Type innerOwnerType = inner.getOwnerType() == null ? null : map(GenericTypeReflector.annotate(inner.getOwnerType()), mappingMode).getType();
